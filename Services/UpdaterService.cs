...
public class DedupeRemoteService
{
    private readonly HttpClient _http;

    public DedupeRemoteService(HttpClient http) => _http = http;

    public async Task<DedupeResult> RunAsync(bool dryRun)
    {
        var payload = JsonSerializer.Serialize(new { dryRun });
        using var content = new StringContent(payload, Encoding.UTF8, "application/json");

        // Ruta de Cloud Functions en Parse/Back4App:
        var resp = await _http.PostAsync("/functions/dedupePacientes", content);
        resp.EnsureSuccessStatusCode();

        var json = await resp.Content.ReadAsStringAsync();

        // El formato de respuesta de Cloud Functions es { "result": {...} }
        var root = JsonSerializer.Deserialize<ParseFunctionResponse<DedupeResult>>(json,
            new JsonSerializerOptions { PropertyNameCaseInsensitive = true });

        return root?.Result ?? new DedupeResult();
    }

    private sealed class ParseFunctionResponse<T>
    {
        public T Result { get; set; }
    }
}

// Opcional: modela el resultado que devuelve tu función (resumen del log)
public class DedupeResult
{
    public bool DryRun { get; set; }
    public int Groups { get; set; }
    public int PatientsScanned { get; set; }
    public int PatientsKept { get; set; }
    public int PatientsDeleted { get; set; }
    public int CitasReasignadas { get; set; }
    public int CitasSnapshotsActualizadas { get; set; }
}
